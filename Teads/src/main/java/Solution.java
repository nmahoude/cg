  ...
  static class EdgeRunner {
    ...
    public void advance() {
      if (currentNode.freeEdges.size() > 1) {
        ...
        Node nextNode;
        Edge edge = currentNode.freeEdges.get(0);
        ...
        if (nextNode.freeEdges.size() == 0) {
          if (nextNode.bestRunner != null) {
            length = Math.max(length, nextNode.bestRunner.length);
        ...
        } else if (nextNode.bestRunner.length < this.length) {
          nextNode.bestRunner.desactivate();
          nextNode.bestRunner = this;
        } else {
          this.desactivate();
    ...
    public static void main(String args[]) {
      ...
      Scanner in = new Scanner(System.in);
        ...
        for (int i=0;i<2*n;i++) {
          ...
          if (node != null) {
            System.err.println("node: "+node.index+", edges: "+node.freeEdges.size());
        ...
        System.err.println("Runners : "+runners.size());
