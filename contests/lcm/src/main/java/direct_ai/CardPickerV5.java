...
public class CardPickerV5 implements Picker {
  ...
  public int pick(Agent agent, CardTriplet triplet) {
    ...
    for (int i=0;i<3;i++) {
      ...
      if (agent.state.side == State.SECOND && (card.isGuard())) {
        faceValue+= 15; // card is better in second position
      }
      if (agent.state.side == State.SECOND && (card.hasCharge())) {
        faceValue+= 15; // card is better in second position
      }
