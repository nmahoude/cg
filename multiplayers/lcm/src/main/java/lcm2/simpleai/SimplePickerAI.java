...
public class SimplePickerAI {
	private static final int MAX_COST_GROUP = 5;
  private Agent me;
	private String action;
	int cardsCountByCosts[] = new int[20];
  ...
  public void run() {
	...
	private void think() {
		...
		cardsCountByCosts[Math.min(bestCard.model.cost,MAX_COST_GROUP)]++;
		action = "PICK "+best;
	...
	private double score(Card card) {
		...
		score += -1.0 * card.model.cost + cardIndexP0[card.model.cardNumber] / (1.0 + coeff); 
...
//		}
	...
	private double coeff(int cost) {
	  ...
	  for (int i=-2;i<=2;i++) {
	    ...
	    total += cardsCountByCosts[Math.min(cost,MAX_COST_GROUP) + i ] * coeffs[2+i];
	  }
	  return total / (manaCurveP0[Math.min(cost,MAX_COST_GROUP)]+1);
  }
  ...
  private int[] manaCurveP0 = { 0, 3, 5, 6, 6, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2};
