...
public class Eval7 implements IEval {
...
//    DECK_COUNT_SCORE("deck card"),
  ...
  public Eval7(double my, double his) {
    wMyBoardCoef = my;
    wHisBoardCoef = his;
  }
...
//          bigCreaturesInHand++;
  ...
  public double evaluateCardEGaetan(Card card) {
    ...
    score += ((card.abilities & Abilities.WARD) != 0) ? 1 : 0;
  ...
  public double evaluateCard(Card card) {
    ...
    double score = 0.0;
    score += 4.22 * Math.abs(card.attack);
    score += 2.34 * Math.abs(card.defense);
    score += ((card.abilities & Abilities.BREAKTHROUGH) != 0) ? 0.23*card.attack : 0;
    score += ((card.abilities & Abilities.LETHAL) != 0) ? 3.78 : 0;
    score += ((card.abilities & Abilities.GUARD) != 0) ? 1.49 : 0;
    score += ((card.abilities & Abilities.WARD) != 0) ? 3.11 : 0;
    score += ((card.abilities & Abilities.DRAIN) != 0) ? 0.36 * card.attack : 0;

    return score;
  }

  
  public static void comparator(Eval7 first, Eval7 second) {
