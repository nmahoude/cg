...
//      System.err.println("champion: " +ag.bestSolution.energy);
      addData(generation, max, min, mean, maxPoints);
    }
  
    
    public void addData(int generation, double max, double min, double mean, int maxPoints) {
      if (max > bestMax) {
        bestMax = max;
      }
      series1.getData().add(new XYChart.Data(""+generation, max));
      series2.getData().add(new XYChart.Data(""+generation, mean));
